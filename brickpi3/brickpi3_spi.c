 ...
 * @write_size: The number of bytes to write (<= BRICKPI3_I2C_MAX_SET_SIZE)
 * @read_buf: The buffer to store the read bp
 * @read_size: The number of bytes to read (<= BRICKPI3_I2C_MAX_GET_SIZE)
 *
 * Returns 0 on success or negative error code.
 */
			  ...
			  u8 *read_buf, u8 read_size)
{
	...
	if (read_size > BRICKPI3_I2C_MAX_GET_SIZE)
		return -EINVAL;
	if (write_size > BRICKPI3_I2C_MAX_SET_SIZE)
		return -EINVAL;
