 ...
 * @analog: pointer to the evb-analog device for accessing data from the
...
static enum hrtimer_restart ev3_output_port_timer_callback(struct hrtimer *timer)
{
	...
	case CON_STATE_NO_DEV:
		new_pin5_mv = evb_analog_out_pin5_value(data->analog, data->id);
	...
	case CON_STATE_PIN5_SETTLE:
		...
		if (data->timer_loop_cnt >= SETTLE_CNT) {
			data->pin5_low_mv = evb_analog_out_pin5_value(data->analog, data->id);
			data->timer_loop_cnt = 0;
			    ...
			    char *buf)
{
	...
	return sprintf(buf, "%d\n",
			evb_analog_out_pin5_value(data->analog, data->id));
}
			  ...
			  struct device *parent)
{
	...
	data->id = pdata->id;
	data->analog = get_evb_analog();
	if (IS_ERR(data->analog)) {
		dev_err(parent, "Could not get evb-analog device.\n");
		err = PTR_ERR(data->analog);
		goto err_request_evb_analog;
	}
...
err_gpio_request_array:
	put_evb_analog(data->analog);
err_request_evb_analog:
	kfree(data);
