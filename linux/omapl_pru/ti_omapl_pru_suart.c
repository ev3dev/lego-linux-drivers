...
static int omapl_pru_suart_get_and_request_irq(struct platform_device *pdev,
...
static int omapl_pru_suart_probe(struct platform_device *pdev)
{
	...
	soft_uart->pru_arm_iomap.pru0_dram_io_addr = devm_ioremap_resource(dev, res);
	if (IS_ERR(soft_uart->pru_arm_iomap.pru0_dram_io_addr))
		return PTR_ERR(soft_uart->pru_arm_iomap.pru0_dram_io_addr);

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pru0-ctrl");
	soft_uart->pru_arm_iomap.pru0_ctrl_io_addr = devm_ioremap_resource(dev, res);
	if (IS_ERR(soft_uart->pru_arm_iomap.pru0_ctrl_io_addr))
		return PTR_ERR(soft_uart->pru_arm_iomap.pru0_ctrl_io_addr);

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pru0-iram");
	soft_uart->pru_arm_iomap.pru0_iram_io_addr = devm_ioremap_resource(dev, res);
	if (IS_ERR(soft_uart->pru_arm_iomap.pru0_iram_io_addr))
		return PTR_ERR(soft_uart->pru_arm_iomap.pru0_iram_io_addr);

#if (!(PRU1_MODE == PRU_MODE_INVALID))
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pru1-dram");
	soft_uart->pru_arm_iomap.pru1_dram_io_addr = devm_ioremap_resource(dev, res);
	if (IS_ERR(soft_uart->pru_arm_iomap.pru1_dram_io_addr))
		return PTR_ERR(soft_uart->pru_arm_iomap.pru1_dram_io_addr);

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pru1-ctrl");
	soft_uart->pru_arm_iomap.pru1_ctrl_io_addr = devm_ioremap_resource(dev, res);
	if (IS_ERR(soft_uart->pru_arm_iomap.pru1_ctrl_io_addr))
		return PTR_ERR(soft_uart->pru_arm_iomap.pru1_ctrl_io_addr);

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "pru1-iram");
	soft_uart->pru_arm_iomap.pru1_iram_io_addr = devm_ioremap_resource(dev, res);
	if (IS_ERR(soft_uart->pru_arm_iomap.pru1_iram_io_addr))
		return PTR_ERR(soft_uart->pru_arm_iomap.pru1_iram_io_addr);
#endif
	...
	soft_uart->mcasp_irq =
		omapl_pru_suart_get_and_request_irq(pdev, "mcasp", soft_uart);
	if (soft_uart->mcasp_irq < 0)
		return soft_uart->mcasp_irq;

	soft_uart->pru_arm_iomap.arm_to_pru0_irq =
		omapl_pru_suart_get_and_request_irq(pdev, "arm-to-pru0", soft_uart);
	if (soft_uart->pru_arm_iomap.arm_to_pru0_irq < 0)
		return soft_uart->pru_arm_iomap.arm_to_pru0_irq;

	soft_uart->pru_arm_iomap.arm_to_pru1_irq =
		omapl_pru_suart_get_and_request_irq(pdev, "arm-to-pru1", soft_uart);
	if (soft_uart->pru_arm_iomap.arm_to_pru1_irq < 0)
		return soft_uart->pru_arm_iomap.arm_to_pru1_irq;

	soft_uart->pru_to_pru_irq =
		omapl_pru_suart_get_and_request_irq(pdev, "pru-to-pru", soft_uart);
	...
	for (i = 0; i < NR_SUART; i++) {
		...
		snprintf(irq_name, 20, "suart%d-tx", i);
		soft_uart->tx_irq[i] = platform_get_irq_byname(pdev, irq_name);
		snprintf(irq_name, 20, "suart%d-rx", i);
		soft_uart->rx_irq[i] = platform_get_irq_byname(pdev, irq_name);
