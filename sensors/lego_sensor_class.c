 ...
 * reading the ``value<N>`` attributes. Values can be converted to floating
 * point if needed by ``value<N>`` / 10.0 ^ ``decimals``.
 *
 * Identifying sensors
 * -------------------
 *
 * Since the name of the ``sensor<N>`` device node does not correspond to the
 ...
 * matching ``driver_name``. Then it will not matter which port a sensor is
 * plugged in to - your program will still work.
 *
 * Sysfs
 * -----
 *
 * Sensors can be found at ``/sys/class/lego-sensor/sensor<N>``, where ``<N>``
 ...
 *    * - ``direct``
 *      - read/write
 *      - Allows direct communication with the sensor for using advanced
 ...
 *        ``seek()`` to set the register to read or write from, then read or
 ...
 *    * - ``driver_name``
 *      - read-only
 *      - Returns the name of the sensor device/driver. See the list of
 *        `supported sensors`_ for a complete list of drivers.
 *
 *    * - ``fw_version``
 *      - read-only
 *      - Returns the firmware version of the sensor if available. Currently
 *        only NXT/I2C sensors support this.
 *
 *    * - ``mode``
 *      - read/write
 *      - Returns the current mode. Writing one of the values returned by
 ...
 *    * - ``num_values``
 *      - read-only
 *      - Returns the number of ``value<N>`` attributes that will return a
 *        valid value for the current mode.
 *
 *    * - ``poll_ms``
 *      - read/write
 *      - Returns the polling period of the sensor in milliseconds. Writing
 ...
 *    * - ``value<N>``
 *      - read-only
 *      - Returns the value or values measured by the sensor. Check
